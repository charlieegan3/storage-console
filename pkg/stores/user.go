...
type UserStore interface {
	...
	AddCredentialsForUser(user *types.User, credentials []webauthn.Credential) error
...
func (udb *UserDB) GetUserByID(id string) (*types.User, error) {
	...
	_, err := udb.db.From("curry_club.users").
		...
		ScanStruct(&user)
	if err != nil {
		return nil, fmt.Errorf("error getting user '%s': %s", id, err.Error())
	}
...
func (udb *UserDB) DeleteUserByID(id string) error {
	...
	if err != nil {
		return fmt.Errorf("error deleting user '%s': %s", id, err.Error())
	}
